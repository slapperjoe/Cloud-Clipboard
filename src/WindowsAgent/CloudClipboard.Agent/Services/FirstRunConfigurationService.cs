...
public sealed class FirstRunConfigurationService : BackgroundService
{
    ...
    public sealed class FirstRunConfigurationService : BackgroundService
    {
        ...
        private async Task BootstrapAsync(CancellationToken cancellationToken)
        {
            using (_activityMonitor.BeginActivity("FirstRunBootstrap"))
            {
                var settingsPath = AgentSettingsPathProvider.GetSettingsPath();
                if (File.Exists(settingsPath))
                {
                ...
                {
                    _logger.LogWarning("No remote configuration exists for owner {OwnerId}. Starting provisioning flow...", ownerId);
                    var provisioningResult = await RunAvaloniaProvisioningDialogAsync(ownerId, principal.SubscriptionId, cancellationToken).ConfigureAwait(false);
                    if (provisioningResult is null)
                    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                IReadOnlyList<AzureSubscriptionInfo> loadedSubscriptions = Array.Empty<AzureSubscriptionInfo>();
                if (!string.IsNullOrWhiteSpace(subscriptionId))
                {
                    loadedSubscriptions = await _metadataProvider.GetSubscriptionsAsync(cancellationToken).ConfigureAwait(false);
                }

                IReadOnlyList<AzureLocationInfo> loadedLocations = Array.Empty<AzureLocationInfo>();
                if (!string.IsNullOrWhiteSpace(subscriptionId))
                {
                    loadedLocations = await _metadataProvider.GetLocationsAsync(subscriptionId, cancellationToken).ConfigureAwait(false);
                }

                subscriptions = loadedSubscriptions;
                locations = loadedLocations;
                if (loadedSubscriptions.Count > 0)
                {
                    initialSubscriptionId = System.Linq.Enumerable.FirstOrDefault(loadedSubscriptions, s => s.IsDefault)?.Id ?? loadedSubscriptions[0].Id;
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                var dialogOptions = new ProvisionBackendDialogOptions(ownerId, initialSubscriptionId, defaults, subscriptions, locations);
                var dialogDependencies = new ProvisionBackendDialogDependencies(_metadataProvider, _provisioningService, _iconProvider);
                return await AvaloniaProvisionDialog.ShowAsync(dialogOptions, dialogDependencies, cancellationToken).ConfigureAwait(false);
            }
            catch (OperationCanceledException)
            {
                return null;
